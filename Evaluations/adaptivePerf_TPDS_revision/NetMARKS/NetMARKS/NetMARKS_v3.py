...
def trigger_migration(qos_target, time_window, namespace, response_code):
    """
    ...
    istio_request_duration_query = f'rate(istio_request_duration_milliseconds_sum{{namespace="{namespace}", response_code="{response_code}"}}[{time_window}m])'
    istio_requests_total_query = f'rate(istio_requests_total{{namespace="{namespace}", response_code="{response_code}"}}[{time_window}m])'
...
def main():
    ...
    # Set application-specific parameters
    global _namespace, _timerange, _step_interval, _qos_garget
    _namespace = 'social-network2'
    _timerange = 1 # adjust the range when at practical experiment
    _step_interval = '1m'
    _qos_target = 300 # in milliseconds # 300, 250, 350
    ...
    # excute the migration, and avoid null node
        ...
        print(f"QoS < {_qos_target}, No migration is needed.")
